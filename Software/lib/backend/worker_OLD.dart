...
class SerialWorker {
  ...
  Future<void> _initializeConnection() async {
    // First try the last known port if available
    ...
    // If no last known port or connection failed, scan all ports
  ...
  try {
    ...
    // Only close the port if verification failed
  ...
  try {
    ...
    subscription = verificationReader.stream.listen(
      (data) {
        try {
          ...
          if (response == "mixlit" && !completer.isCompleted) {
    ...
    // Send query multiple times with delays
    for (var i = 0; i < 3; i++) {
    ...
    // Set timeout timer
    timeoutTimer = Timer(Duration(milliseconds: 1000), () {
    ...
    _reader!.stream.listen(
      (data) {
        if (data.isNotEmpty) {
          ...
          // Ensure we have a sendPort before forwarding
